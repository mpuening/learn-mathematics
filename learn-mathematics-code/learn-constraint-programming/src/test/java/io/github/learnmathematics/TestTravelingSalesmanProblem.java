...
public class TestTravelingSalesmanProblem {
	...
	public void testTravelingSalesmanProblem() {
		...
		// FIXME, how does one know what is a proper value for max?
		int max = 350;
		// For each city, the distance to the succ visited one
		IntVar[] dist = model.intVarArray("dist", C, 0, max);
		// Total distance of the route
		IntVar totalDistance = model.intVar("Total distance", 0, max * C);
		...
		model.sum(dist, "=", totalDistance).post();

		model.setObjective(Model.MINIMIZE, totalDistance);
		Solver solver = model.getSolver();
		solver.setSearch(Search.intVarSearch(new MaxRegret(), new IntDomainMin(), dist));
		...
		while (solver.solve()) {
			...
			finalAnswer = totalDistance.getValue();
		}
		assertEquals(2085, finalAnswer);
	}
}
